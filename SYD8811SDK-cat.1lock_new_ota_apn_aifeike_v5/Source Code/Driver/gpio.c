...
static void (*p_gpio_callback)(void);
...
//管脚初始化,如果马达需要转动，CAT1_WAKE_UP也需置高
void hw_gpio_init(void)
{
	GPIO_Pin_Set(U32BIT(CAT1_WAKE_UP) | U32BIT(WatchDog) | U32BIT(CAT1_PWR_KEY) | U32BIT(GNSS_PWR_EN));  
	GPIO_Pin_Set(U32BIT(GNSS_PWR_EN));	

	GPIO_Pin_Clear(U32BIT(CAP_BOOST));

//  GPIO_Pin_Clear(U32BIT(CAT1_WAKE_UP) | U32BIT(CAT1_PWR_KEY));
  
	GPIO_Pin_Clear(U32BIT(GREEN_LED) | U32BIT(BLUE_LED) | U32BIT(MotorCtrl_1) | U32BIT(MotorCtrl_2) | U32BIT(CAT1_PWR_CTRL));
  DelayMS(250); 
  GPIO_Pin_Set(U32BIT(GREEN_LED) | U32BIT(BLUE_LED));   
	DelayMS(250); 
	GPIO_Pin_Clear(U32BIT(GREEN_LED) | U32BIT(BLUE_LED)); 	
  DelayMS(250); 
  GPIO_Pin_Set(U32BIT(GREEN_LED) | U32BIT(BLUE_LED));  	
...
{	
	...
	p_gpio_callback = ((void (*)(void))p_callback);
