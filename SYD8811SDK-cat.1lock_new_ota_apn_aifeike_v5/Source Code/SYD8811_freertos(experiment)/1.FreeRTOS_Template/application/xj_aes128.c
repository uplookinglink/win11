...
int addRoundKey(uint8_t (*state)[4], const uint32_t *key) 
{
    ...
    for (int i = 0; i < 4; ++i) 
	{
        for (int j = 0; j < 4; ++j) 
		{
			//把 uint32 key[4] 先转换为矩阵 uint8 k[4][4]
...
int shiftRows(uint8_t (*state)[4]) 
{
    ...
    for (int i = 0; i < 4; ++i) 
	{
    	//便于行循环移位，先把一行4字节拼成uint_32结构，
    	//移位后再转成独立的4个字节uint8_t
...
int aes128_encrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *pt, uint8_t *ct, uint32_t len) {
...
//AES128解密	，参数要求同加密
int aes128_decrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *ct, uint8_t *pt, uint32_t len) {
    AesKey aesKey;
    uint8_t *pos = pt;
