...
// table is only used by the (Inv)Cx_MixColumns steps.
...
//aes_decrypt_ccm(ciphertext,20,aesBuf.associated_data,14,aesBuf.nonce,12,plaintext,&aesBuf.plaintext_len,4,mac_c, 
//aesBuf.key, 128);
                    ...
                    WORD mac_len, int *mac_auth, const BYTE key_str[], int keysize)
{
	...
	buf = (BYTE*)pvPortMalloc(assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48);
	if (! buf)
		return(FALSE);
...
void ccm_prepare_first_format_blk(BYTE buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const BYTE nonce[], int nonce_len)
{
	...
	if (assoc_len > 0)
		buf[0] += 0x40;
	//printf("buf0 %x %d",buf[0],payload_len_store_size);
...
// Performs the Cx_AddRoundKey step. Each round has its own pre-generated 16-byte key in the
...
void Cx_AddRoundKey(BYTE state[][4], const WORD w[])
{
	BYTE subkey[4];
...
// Performs the Cx_SubBytes  step. All bytes in the state are substituted with a
// pre-calculated value from a lookup table.
void Cx_SubBytes (BYTE state[][4])
{
...
void Cx_InvSubBytes(BYTE state[][4])
{
...
// Performs the Cx_ShiftRows step. All rows are shifted cylindrically to the left.
void Cx_ShiftRows(BYTE state[][4])
{
	int t;
...
void Cx_InvShiftRows(BYTE state[][4])
{
	int t;
...
void Cx_MixColumns(BYTE state[][4])
{
	BYTE col[4];
...
void Cx_InvMixColumns(BYTE state[][4])
{
	BYTE col[4];
...
void aes_encrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	...
	// The last round does not perform the Cx_MixColumns step.
	Cx_AddRoundKey(state,&key[0]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[4]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[8]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[12]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[16]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[20]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[24]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[28]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[32]);
	Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[36]);
	if (keysize != 128) {
		Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[40]);
		Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[44]);
		if (keysize != 192) {
			Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[48]);
			Cx_SubBytes (state); Cx_ShiftRows(state); Cx_MixColumns(state); Cx_AddRoundKey(state,&key[52]);
			Cx_SubBytes (state); Cx_ShiftRows(state); Cx_AddRoundKey(state,&key[56]);
		}
		else {
			Cx_SubBytes (state); Cx_ShiftRows(state); Cx_AddRoundKey(state,&key[48]);
		}
	}
	else {
		Cx_SubBytes (state); Cx_ShiftRows(state); Cx_AddRoundKey(state,&key[40]);
	}
...
void aes_decrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	...
	if (keysize > 128) {
		if (keysize > 192) {
			Cx_AddRoundKey(state,&key[56]);
			Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[52]);Cx_InvMixColumns(state);
			Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[48]);Cx_InvMixColumns(state);
		}
		else {
			Cx_AddRoundKey(state,&key[48]);
		}
		Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[44]);Cx_InvMixColumns(state);
		Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[40]);Cx_InvMixColumns(state);
	}
	else {
		Cx_AddRoundKey(state,&key[40]);
	}
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[36]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[32]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[28]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[24]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[20]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[16]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[12]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[8]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[4]);Cx_InvMixColumns(state);
	Cx_InvShiftRows(state);Cx_InvSubBytes(state);Cx_AddRoundKey(state,&key[0]);
